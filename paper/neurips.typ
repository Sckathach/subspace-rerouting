...
#let format-author-name(author, affl2idx, affilated: false) = {
  ...
  if author.name == "Thomas Winninger" {
    return box([#result #footnote("Correspondance to thomas.winninger@telecom-sudparis.eu")])
  } else {
    return box(result)
  }
...
#let make-two-authors(authors, affls, affl2idx) = {
  let row = authors
    ...
    .map(it => box(it))
  return align(center, grid(columns: (1fr, 1fr, 1fr), gutter: 2em, ..row))
}
